        ...
        if(s instanceof Player) {
            ...
            if(u.isRoleplay()) {
                if(u.getRoleplayName() == null) {
                    ...
                    if(args.length == 0) {
                        API.sendErrorMessage(p,"§4Fehler: §c/shout [Nachricht]");
                    } else {
                        String MSG="";
                        ...
                        String RPM = "§6RP §8● §e"+Job+"§8 ● §6"+u.getRoleplayName()+"§8 (SCHREIT) » §f"+MSG.toUpperCase();
                        for(Player all: Bukkit.getOnlinePlayers()) {
                            if (p.getLocation().getWorld().equals(all.getLocation().getWorld()) && p.getLocation().distance(all.getLocation()) <= 45) {
                                all.sendMessage(RPM);
                            }
                        }
